    ...
    pub async fn dispatch(&self, call: ToolCall) -> ToolResult {
        match self.tools.get(&call.name) {
            ...
            None => ToolResult::error(
                &call.id,
                &call.name,
                format!("Tool '{}' not found. Available: {:?}", call.name, self.tool_names()),
            ),
        }
    }
...
impl EngineTool for ReadFileTool {
    ...
    async fn execute(&self, call: &ToolCall) -> ToolResult {
        ...
        match std::fs::read_to_string(path) {
            ...
            Err(e) => ToolResult::error(&call.id, self.name(), format!("Cannot read '{path}': {e}")),
        }
    }
}
...
impl EngineTool for WriteFileTool {
    ...
    async fn execute(&self, call: &ToolCall) -> ToolResult {
        ...
        match std::fs::write(path, content) {
            ...
            Err(e) => ToolResult::error(&call.id, self.name(), format!("Cannot write '{path}': {e}")),
        }
    }
}
...
impl EngineTool for ListFilesTool {
    ...
    async fn execute(&self, call: &ToolCall) -> ToolResult {
        let path = require_str!(call, "path");
        let recursive = call.parameters.get("recursive").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        if recursive {
            ...
            match std::fs::read_dir(path) {
                ...
                Err(e) => {
                    return ToolResult::error(&call.id, self.name(), format!("Cannot list '{path}': {e}"))
                }
            }
        }
...
impl EngineTool for SearchFilesTool {
    ...
    async fn execute(&self, call: &ToolCall) -> ToolResult {
        ...
        let case_sensitive = call.parameters.get("case_sensitive").and_then(|v| v.as_bool()).unwrap_or(false);
        let max_results = call.parameters.get("max_results").and_then(|v| v.as_u64()).unwrap_or(50) as usize;
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if matches!(ext, "png" | "jpg" | "jpeg" | "gif" | "bmp" | "wasm" | "bin" | "dll" | "exe") {
                continue;
            }
            if let Ok(content) = std::fs::read_to_string(entry.path()) {
                for (line_num, line) in content.lines().enumerate() {
                    let cmp_line = if case_sensitive { line.to_string() } else { line.to_lowercase() };
...
impl EngineTool for DeleteFileTool {
    ...
    async fn execute(&self, call: &ToolCall) -> ToolResult {
        ...
        match result {
            ...
            Err(e) => ToolResult::error(&call.id, self.name(), format!("Cannot delete '{path}': {e}")),
        }
    }
}
...
impl EngineTool for GetSceneTreeTool {
    fn name(&self) -> &str { "get_scene_tree" }
    fn description(&self) -> &str {
        "Get the current scene tree structure as a JSON hierarchy."
    }
    fn category(&self) -> ToolCategory { ToolCategory::SceneTree }
...
impl EngineTool for CreateNodeTool {
    ...
    async fn execute(&self, call: &ToolCall) -> ToolResult {
        ...
        let parent = call.parameters.get("parent").and_then(|v| v.as_str()).unwrap_or("/Root");
        ToolResult::ok_text(
            &call.id,
            self.name(),
            format!("Created {node_type} node '{name}' under '{parent}' (pending runtime integration)"),
        )
    }
}
...
impl EngineTool for SetNodePropertyTool {
    fn name(&self) -> &str { "set_node_property" }
    fn description(&self) -> &str {
...
impl EngineTool for GetNodePropertyTool {
    fn name(&self) -> &str { "get_node_property" }
    fn description(&self) -> &str {
...
impl EngineTool for ExecuteCommandTool {
    ...
    fn category(&self) -> ToolCategory { ToolCategory::Editor }
    fn requires_approval(&self) -> bool { true }
    fn parameters_schema(&self) -> Value {
        serde_json::json!({
            "type": "object",
...
impl EngineTool for GetProjectInfoTool {
    fn name(&self) -> &str { "get_project_info" }
    fn description(&self) -> &str {
...
mod tests {
    ...
    fn test_registry_has_all_tools() {
        ...
        assert!(registry.len() >= 12, "Expected at least 12 tools, got {}", registry.len());
        // 验证关键工具已注册
    ...
    fn test_all_schemas() {
        ...
        for schema in &schemas {
            ...
            assert!(schema["description"].is_string(), "Schema missing 'description'");
        }
    }
