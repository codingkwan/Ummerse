...
pub use material::{AlphaMode, Material, MaterialId, PbrMaterial, PbrMaterialUniform, UnlitMaterial, DirectionalLightUniform};
pub use mesh::{GpuMesh, GpuMesh2d, MeshBuilder, MeshData, MeshData2d, Vertex2d, Vertex3d};
pub use pipeline::{
    PostProcessPipeline, RenderPipeline2d, RenderPipeline3d,
    SPRITE_SHADER_WGSL, PBR_SHADER_WGSL, POST_PROCESS_SHADER_WGSL,
};
...
pub struct RenderContext {
    ...
    /// 深度缓冲
    pub depth_texture: texture::GpuTexture,
    /// HDR 渲染目标（若启用 HDR）
    pub hdr_texture: Option<texture::GpuTexture>,
}
...
impl RenderContext {
    ...
    pub async fn new(
        ...
        let depth_texture = texture::GpuTexture::create_depth_texture(
            &device,
            width.max(1),
            height.max(1),
        ...
        let hdr_texture = if renderer_config.hdr {
            Some(texture::GpuTexture::create_hdr_target(
                &device,
                width.max(1),
                height.max(1),
                renderer_config.sample_count,
    ...
    pub fn resize(&mut self, width: u32, height: u32) {
        ...
        // 重建深度缓冲
        self.depth_texture = texture::GpuTexture::create_depth_texture(
            &self.device,
            w,
            h,
        ...
        if self.renderer_config.hdr {
            self.hdr_texture = Some(texture::GpuTexture::create_hdr_target(
                &self.device,
                w,
                h,
                self.renderer_config.sample_count,
...
impl SpriteBatch {
    ...
    pub fn push_sprite(
        ...
        for (i, &corner) in corners.iter().enumerate() {
            ...
            self.vertices.push(mesh::Vertex2d::new(world_pos, uvs[i], color));
        }
