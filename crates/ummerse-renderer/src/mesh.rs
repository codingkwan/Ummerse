...
impl MeshBuilder {
    ...
    pub fn quad(size: f32) -> Self {
        ...
        let mut b = Self::new();
        // 4 个顶点
        b.data.add_vertex(Vec3::new(-h, -h, 0.0), Vec3::Z, Vec2::new(0.0, 1.0));
        b.data.add_vertex(Vec3::new(h, -h, 0.0), Vec3::Z, Vec2::new(1.0, 1.0));
        b.data.add_vertex(Vec3::new(h, h, 0.0), Vec3::Z, Vec2::new(1.0, 0.0));
        b.data.add_vertex(Vec3::new(-h, h, 0.0), Vec3::Z, Vec2::new(0.0, 0.0));
    ...
    pub fn cylinder(radius: f32, height: f32, segments: u32) -> Self {
        ...
        // 上下顶面中心
        let top_center = b.data.add_vertex(Vec3::new(0.0, h, 0.0), Vec3::Y, Vec2::new(0.5, 0.5));
        let bot_center = b.data.add_vertex(Vec3::new(0.0, -h, 0.0), Vec3::NEG_Y, Vec2::new(0.5, 0.5));
        ...
        for i in 0..segments {
            ...
            b.data.add_vertex(Vec3::new(nx * radius, h, nz * radius), Vec3::Y, top_uv);
            b.data.add_vertex(Vec3::new(nx * radius, -h, nz * radius), Vec3::NEG_Y, bot_uv);
            // 侧面顶点（带侧向法线）
            b.data.add_vertex(Vec3::new(nx * radius, h, nz * radius), Vec3::new(nx, 0.0, nz), top_uv);
            b.data.add_vertex(Vec3::new(nx * radius, -h, nz * radius), Vec3::new(nx, 0.0, nz), bot_uv);
        }
        ...
        for i in 0..segments {
            ...
            // 侧面（2 个三角形）
            b.data.add_triangle(cur_side_top, next_side_top, cur_side_bot);
            b.data.add_triangle(next_side_top, next_side_bot, cur_side_bot);
        }
        b
    }
    ...
    pub fn cone(radius: f32, height: f32, segments: u32) -> Self {
        let mut b = Self::new();
        let tip = b.data.add_vertex(Vec3::new(0.0, height, 0.0), Vec3::Y, Vec2::new(0.5, 0.5));
        let center = b.data.add_vertex(Vec3::ZERO, Vec3::NEG_Y, Vec2::new(0.5, 0.5));
        let step = 2.0 * PI / segments as f32;
    ...
    pub fn plane(width: f32, depth: f32, divisions_x: u32, divisions_z: u32) -> Self {
        ...
        for z in 0..rows {
            for x in 0..cols {
                ...
                let px = -hw + tx * width;
                let pz = -hd + tz * depth;
                b.data.add_vertex(Vec3::new(px, 0.0, pz), Vec3::Y, Vec2::new(tx, tz));
            }
        }
