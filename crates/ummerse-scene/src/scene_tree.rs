...
impl SceneTree {
    ...
    fn rebuild_path_cache(&mut self) {
        ...
        if let Some(root_id) = self.root {
            self.rebuild_path_recursive(root_id, "/".to_string());
        }
    }

    fn rebuild_path_recursive(&mut self, id: NodeId, parent_path: String) {
        ...
        for child_id in children {
            self.rebuild_path_recursive(child_id, path.clone());
        }
    }
    ...
    fn compute_path(&self, parent_id: Option<NodeId>, name: &str) -> String {
        if let Some(pid) = parent_id {
            // 查找父路径
            if let Some((parent_path, _)) = self.path_cache.iter().find(|(_, &id)| id == pid) {
