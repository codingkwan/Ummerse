...
impl AudioPlayback {
    ...
    pub fn update(&mut self, delta: f32) {
        ...
        if self.params.fade_in > 0.0 && self.current_volume < self.params.volume {
            self.current_volume = (self.current_volume + delta / self.params.fade_in * self.params.volume)
...
impl AudioPlayer {
    ...
    /// 播放音频，返回播放 ID
    pub fn play(&mut self, audio_path: impl Into<String>, params: PlaybackParams, duration: f32) -> PlaybackId {
    ...
    pub fn update(&mut self, delta: f32) {
        ...
        self.playbacks.retain(|pb| {
            pb.state != PlaybackState::Finished && pb.state != PlaybackState::Stopped
        });
    }
