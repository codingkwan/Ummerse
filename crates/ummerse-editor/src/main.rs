...
use ummerse_editor::{EditorApp, EditorMode, EDITOR_VERSION};
...
/// 编辑器启动系统
fn editor_startup_system(
    editor: Res<EditorStateResource>,
    tools: Res<EditorToolRegistryResource>,
) {
    let app = &editor.0;

    tracing::info!(
        project  = app.project.as_ref().map(|p| p.config.name.as_str()).unwrap_or("none"),
...
/// 编辑器每帧更新系统
fn editor_update_system(mut editor: ResMut<EditorStateResource>, time: Res<Time>) {
...
fn handle_editor_shortcuts(
    ...
    if keyboard.just_pressed(KeyCode::Escape)
        && editor.0.mode == EditorMode::GamePreview
    {
